# He decidido no añadir elementos como imagenes extra o efectos de sonido ya que se necesitarian cambiar varias rutas, complejizando el proceso de iniciar y correr el programa
import pygame
import random
...
imagenes_dados = [pygame.image.load(f"C:/Users/tobar/Downloads/dados/imagenes/dados/dado{i}.png") for i in range(1, 7)]
...
def evaluar_ronda():
    ...
    # Situaciones de jugador
    if jugador_suma>10:
        resultado=f"Perdiste debido a que te pasaste de 10."
    elif jugador_suma==10:
        resultado=f"Felicitaciones! Ganaste con un 10 exacto!"
    elif jugador_suma>cpu_suma:
        resultado=f"Ganaste con un {jugador_suma}."
    # Situaciones de CPU
    if cpu_suma>10:
        resultado=f"Ganaste ya que la CPU se paso de 10."
    elif cpu_suma==10:
        resultado=f"Oh No! Perdiste ya que la CPU tiene un 10 exacto!"
    elif cpu_suma>jugador_suma:
        resultado=f"Perdiste con un {jugador_suma}. La CPU tenia {cpu_suma}."
    # Situaciones de ambos
    if jugador_suma==cpu_suma:
        resultado=f"No gano nadie ya que hubo un empate de {jugador_suma}."
    if jugador_suma>10 and cpu_suma>10:
        resultado=f"No gano nadie ya que ambos se pasaron de 10."
    turno = "fin"
